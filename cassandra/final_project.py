...
# -For a Hackernews use case, I selected MongoDB database because of its flexible design which allows for creating a nesting hierarchy of sub-documents (e.g. sub-comments) in sub-documents (e.g. comments) in documents (e.g. posts).
#
# Explain what will happen if coffee is spilled on one of the servers in your cluster, causing it to go down.
# -MongoDB uses replication to create redundancies for failover. If the affected server is a primary node, then a secondary server will be elected to replace it as the master.
#
# What data is it not ok to lose in your app? What can you do in your commands to mitigate the risk of lost data?
# Data containing article information (content, score, comments) and user information (e.g. favorites, login credentials, email) can be costly.  To mitigate the risk of lost data, article and user information can be replicated into backups.  These processes can be automated to ensure recovery in the case of lost data.
...
articles.insert([{'_id':1, 
		 ...
		 {'_id':15, 
		  ...
		  'link':'https://en.wikipedia.org/wiki/The_Oregon_Trail_(video_game)',
		  'score_a':1,
		  'op':'peterparker',
		  'comments':{}}
		  ])
...
# Action 2: steverogers visits the homepage, returning the top 5 articles
homepage = articles.aggregate([{'$sort':{'score_a':-1}}]).limit(5)
print(list(homepage))
print("hi")

# Action 3: steverogers clicks on the top article
